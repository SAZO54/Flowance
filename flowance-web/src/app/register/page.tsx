    ...
    <section className={`${styles.formPane} ${styles.registerPane}`}>
      ...
      <div className={styles.formWrap}>
        ...
        <form onSubmit={handleSubmit} className={`${styles.form} ${styles.registerForm}`} noValidate>
          <label><span>表示名 <i className="required-symbol">※</i></span><div className={styles.inputWrap}><UserRound size={18}/><input value={displayName} onChange={event => setDisplayName(event.target.value)} name="displayName" autoComplete="name" maxLength={100} placeholder="山田 花子" required disabled={submitting}/></div></label>
          <label><span>組織名 <i className="required-symbol">※</i></span><div className={styles.inputWrap}><Building2 size={18}/><input value={organizationName} onChange={event => setOrganizationName(event.target.value)} name="organizationName" autoComplete="organization" maxLength={150} placeholder="Flowance Studio" required disabled={submitting}/></div></label>
          <label><span>メールアドレス <i className="required-symbol">※</i></span><div className={styles.inputWrap}><Mail size={18}/><input type="email" value={email} onChange={event => setEmail(event.target.value)} name="email" autoComplete="email" maxLength={254} placeholder="you@example.com" required disabled={submitting}/></div></label>
          <label><span>パスワード <i className="required-symbol">※</i></span><div className={styles.inputWrap}><LockKeyhole size={18}/><input type="password" value={password} onChange={event => setPassword(event.target.value)} name="password" autoComplete="new-password" minLength={12} maxLength={128} placeholder="12文字以上で入力" required disabled={submitting}/></div></label>
