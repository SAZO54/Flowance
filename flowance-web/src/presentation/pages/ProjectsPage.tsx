...
export function ProjectsPage({events, projects, projectDetails}:{events: EventItem[]; projects: Project[]; projectDetails: Record<string, ProjectDetail>}) {
  ...
  return <div className="projects-page">
    <div className="projects-titlebar">
      ...
      <button className="add-btn"><Plus size="1.0625rem"/>新しい案件</button>
    </div>

    <div className="project-summary">
      <article><span className="summary-icon"><BriefcaseBusiness size="1.125rem"/></span><div><p>進行中の案件</p><strong>3<small> 件</small></strong></div></article>
      <article><span className="summary-icon"><CircleDollarSign size="1.125rem"/></span><div><p>契約金額合計</p><strong>¥1,550,000</strong></div></article>
    ...
    <section className="projects-panel">
      <div className="projects-toolbar">
        <div className="project-tabs"><button className={status==='all'?'active':''} onClick={()=>setStatus('all')}>すべて <b>{projects.length}</b></button><button className={status==='active'?'active':''} onClick={()=>setStatus('active')}>進行中 <b>2</b></button><button className={status==='attention'?'active':''} onClick={()=>setStatus('attention')}>確認待ち <b>1</b></button></div>
      ...
      <div className="project-card-grid">{filteredProjects.map(project => {
        ...
        return <article className="project-card" key={project.id}>
          ...
          <dl><div><dt>契約金額</dt><dd>{detail.budget}</dd></div><div><dt>期限</dt><dd>{detail.deadline}</dd></div></dl>
