...
export function SchedulePage(props: SchedulePageProps) {
  ...
  return <div className="schedule-page">
    ...
    {form && <div className="modal-backdrop" onMouseDown={() => setForm(null)}><div className="modal" onMouseDown={event => event.stopPropagation()}><div className="modal-head"><div><p>{form.mode === 'edit' ? 'EDIT SCHEDULE' : 'NEW SCHEDULE'}</p><h2>{form.mode === 'edit' ? '予定を編集' : form.showDateInTitle ? `${displayDate(selectedDate)}の予定を追加` : '予定を追加'}</h2></div><button aria-label="閉じる" onClick={() => setForm(null)}><X/></button></div><form key={form.mode === 'edit' ? form.schedule.id : form.startAt} onSubmit={submit}>{formError && <p className="form-error" role="alert">{formError}</p>}<label><span className="field-label">案件 <i className="required-symbol">※</i></span><select name="project" required defaultValue={form.mode === 'edit' ? form.schedule.projectId : ''}><option value="" disabled>案件を選択</option>{projects.map(project => <option value={project.id} key={project.id}>{project.name}</option>)}</select></label><label><span className="field-label">作業内容 <i className="required-symbol">※</i></span><input name="title" required maxLength={150} defaultValue={form.mode === 'edit' ? form.schedule.title : ''}/></label><div className="schedule-form-row"><label><span className="field-label">開始日時 <i className="required-symbol">※</i></span><DateTimePickerInput name="startAt" required defaultValue={form.startAt} ariaLabel="開始日時"/></label><label><span className="field-label">終了日時 <i className="required-symbol">※</i></span><DateTimePickerInput name="endAt" required defaultValue={form.endAt} ariaLabel="終了日時"/></label></div><label><span className="field-label">メモ</span><textarea name="notes" rows={3} defaultValue={form.mode === 'edit' ? form.schedule.notes ?? '' : ''}/></label>{form.mode === 'edit' && form.schedule.isGenerated && <p className="schedule-generated-note">編集すると、この予定は今後の自動生成で上書きされません。</p>}<div className="modal-actions">{form.mode === 'edit' && <button className="schedule-delete" type="button" disabled={isMutating} onClick={() => void deleteCurrent()}><Trash2 size="1rem"/>削除</button>}<button type="button" disabled={isMutating} onClick={() => setForm(null)}>キャンセル</button><button type="submit" disabled={isMutating}>{isMutating ? '処理中…' : form.mode === 'edit' ? '変更を保存' : '予定に追加'}</button></div></form></div></div>}
  </div>
}
