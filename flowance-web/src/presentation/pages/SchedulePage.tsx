...
const zonedParts = (value: string) => new Intl.DateTimeFormat('en-CA', {
  timeZone: TZ,
  year: 'numeric',
  month: '2-digit',
  day: '2-digit',
...
const eventTime = (value: string) => new Intl.DateTimeFormat('ja-JP', {
  timeZone: TZ,
  hour: '2-digit',
  minute: '2-digit',
  hourCycle: 'h23',
...
const nextHourMinuteInTokyo = () => {
  const parts = zonedParts(new Date().toISOString())
...
export function SchedulePage(props: SchedulePageProps) {
  const {
    events, projects, filterProjectId, period, anchorDate, selectedDate,
  ...
  const selectedEvents = visible
    .filter(event => eventDay(event.startAt) === selectedDate)
    .sort((left, right) => left.startAt.localeCompare(right.startAt))
  const label = period === 'week'
    ? `${displayDate(weekStart(anchorDate))} — ${displayDate(shiftDays(weekStart(anchorDate), 6))}`
  ...
  const openEdit = async (event: CalendarEvent) => {
    ...
    try {
      ...
      onSelectedDateChange(eventDay(schedule.scheduledStartAt))
      setForm({
        mode: 'edit',
        schedule,
        startAt: localDateTimeFromIso(schedule.scheduledStartAt),
        endAt: localDateTimeFromIso(schedule.scheduledEndAt),
      })
    } catch (cause) {
  ...
  return <div className="schedule-page">
    ...
    <div className="schedule-content-grid">
        ...
        {!hasLoaded
          ...
          : isLoading
            ...
            : error
              ...
              : period === 'week'
                ? <WeekCalendar events={visible} projects={projects} anchorDate={anchorDate} selectedDate={selectedDate} onSelectDate={onSelectedDateChange} onSelectSlot={openCreate} onOpenSchedule={openEdit} openingScheduleId={openingScheduleId}/>
                : <MonthView events={visible} projects={projects} anchorDate={anchorDate} selectedDate={selectedDate} onSelectDate={onSelectedDateChange} onCreateForDate={date => openCreate(date, nextHourMinuteInTokyo())} onOpenSchedule={openEdit} openingScheduleId={openingScheduleId}/>
        }
      </section>
      <aside className="day-agenda">
        ...
        <div className="agenda-list">{!hasLoaded || isLoading ? null : selectedEvents.length ? selectedEvents.map(event => <AgendaEvent key={`${event.type}-${event.id}`} event={event} project={projectOf(event, projects)} isOpening={openingScheduleId === event.id} onOpenSchedule={openEdit}/>) : <p className="agenda-empty">この日の予定・実績はありません。</p>}</div>
        <button disabled={!hasLoaded || !projects.length} onClick={() => openCreate()}><Plus size="1rem"/>この日に予定を追加</button>
      </aside>
    </div>
    {form && <div className="modal-backdrop" onMouseDown={() => setForm(null)}><div className="modal" onMouseDown={event => event.stopPropagation()}><div className="modal-head"><div><p>{form.mode === 'edit' ? 'EDIT SCHEDULE' : 'NEW SCHEDULE'}</p><h2>{form.mode === 'edit' ? '予定を編集' : form.showDateInTitle ? `${displayDate(selectedDate)}の予定を追加` : '予定を追加'}</h2></div><button aria-label="閉じる" onClick={() => setForm(null)}><X/></button></div><form key={form.mode === 'edit' ? form.schedule.id : form.startAt} onSubmit={submit}>{formError && <p className="form-error" role="alert">{formError}</p>}<label>案件<select name="project" required defaultValue={form.mode === 'edit' ? form.schedule.projectId : ''}><option value="" disabled>案件を選択</option>{projects.map(project => <option value={project.id} key={project.id}>{project.name}</option>)}</select></label><label>作業内容<input name="title" required maxLength={150} defaultValue={form.mode === 'edit' ? form.schedule.title : ''}/></label><div className="schedule-form-row"><label>開始日時<DateTimePickerInput name="startAt" required defaultValue={form.startAt} ariaLabel="開始日時"/></label><label>終了日時<DateTimePickerInput name="endAt" required defaultValue={form.endAt} ariaLabel="終了日時"/></label></div><label>メモ<textarea name="notes" rows={3} defaultValue={form.mode === 'edit' ? form.schedule.notes ?? '' : ''}/></label>{form.mode === 'edit' && form.schedule.isGenerated && <p className="schedule-generated-note">編集すると、この予定は今後の自動生成で上書きされません。</p>}<div className="modal-actions">{form.mode === 'edit' && <button className="schedule-delete" type="button" disabled={isMutating} onClick={() => void deleteCurrent()}><Trash2 size="1rem"/>削除</button>}<button type="button" disabled={isMutating} onClick={() => setForm(null)}>キャンセル</button><button type="submit" disabled={isMutating}>{isMutating ? '処理中…' : form.mode === 'edit' ? '変更を保存' : '予定に追加'}</button></div></form></div></div>}
  </div>
}

function AgendaEvent({event, project, isOpening, onOpenSchedule}: {event: CalendarEvent; project?: ScheduleProject; isOpening: boolean; onOpenSchedule: (event: CalendarEvent) => void}) {
...
export function WeekCalendar({events, projects, anchorDate, selectedDate, onSelectDate, onSelectSlot, onOpenSchedule, openingScheduleId}: WeekCalendarProps) {
  const start = weekStart(anchorDate)
  const days = Array.from({length: 7}, (_, index) => shiftDays(start, index))
  const starts = events.map(event => eventMinute(event.startAt))
  const ends = events.map(event => eventMinute(event.endAt))
  ...
  return <div className="week-calendar"><div className="calendar-header"><div/>{days.map(day => { const date = parseDate(day); return <button type="button" className={`${day === selectedDate ? 'today ' : ''}${date.getDay() === 0 || date.getDay() === 6 ? 'weekend' : ''}`} key={day} onClick={() => onSelectDate(day)}><span>{SHORT_DAYS[date.getDay()]}</span><strong>{date.getDate()}</strong></button> })}</div><div className="calendar-body" style={{height: bodyHeight}}><div className="times" style={{gridTemplateRows: `repeat(${hours.length}, 3.25rem)`}}>{hours.map(hour => <span key={hour}>{pad(hour)}:00</span>)}</div><div className="days-grid">{days.map(day => { const date = parseDate(day); return <div className={`day-col ${date.getDay() === 0 || date.getDay() === 6 ? 'weekend' : ''}`} key={day}>{hours.map(hour => <button className="calendar-slot" type="button" key={hour} aria-label={`${displayDate(day)} ${pad(hour)}時の予定を追加`} onClick={mouseEvent => { const minuteOffset = mouseEvent.nativeEvent.offsetY >= mouseEvent.currentTarget.clientHeight / 2 ? 30 : 0; onSelectSlot(day, hour * 60 + minuteOffset) }}/>)}{events.filter(event => eventDay(event.startAt) === day).map(event => { const project = projectOf(event, projects); const color = project?.labelColor ?? '#75A8C7'; const from = eventMinute(event.startAt); const to = eventMinute(event.endAt); const editable = event.type === 'SCHEDULE'; return <button type="button" className={`event ${editable ? 'editable' : 'actual-event'} ${openingScheduleId === event.id ? 'opening' : ''}`} key={`${event.type}-${event.id}`} aria-label={editable ? `${event.title}を編集` : `${event.title}の実績`} aria-busy={openingScheduleId === event.id} onClick={() => editable ? void onOpenSchedule(event) : onSelectDate(day)} style={{top: `${((from - minHour * 60) / 60) * 3.25 + .3125}rem`, height: `${Math.max(.85, ((to - from) / 60) * 3.25 - .5)}rem`, background: softColor(color), borderColor: color, color}}><span className="event-meta"><em>{editable ? '予定' : '実績'}</em><b>{eventTime(event.startAt)}–{eventTime(event.endAt)}</b></span><strong className="event-title">{event.title}</strong></button> })}</div> })}</div></div></div>
}
...
export function MonthView({events, projects, anchorDate, selectedDate, onSelectDate, onCreateForDate, onOpenSchedule, openingScheduleId}: MonthViewProps) {
  ...
  return <div className="month-view">{days.map(day => {
    ...
    const items = events.filter(event => eventDay(event.startAt) === day)
