...
| 種別 | 説明 |
| --- | --- |
| DEFAULT | システムが名称と安定 ID から自動生成する初期アイコン |
...
DEFAULT アイコンは、名称と安定 ID から生成する。

- 表示文字は名称から生成する。
- 背景色は client_id または project_id などの安定 ID をハッシュ化し、固定カラーパレットから決定する。
- 文字色は背景色とのコントラスト比から白または濃色を選択する。
- 生成した背景色は DB に保存する。
- 名称変更時に背景色は再生成しない。
...
- 空文字の場合は代替文字を使用する。
- 日本語名は先頭の有効文字を優先する。
- 英数字名は単語頭文字を最大 2 文字まで使用する。
- 表示文字は 1 から 4 文字以内に収める。
- 絵文字、制御文字、不可視文字は除外する。

例:

| 名称 | default_icon_text |
| --- | --- |
| Nova Works | NW |
| Akari Design | AD |
| SaaSリニューアル | S |
...
背景色は名称ではなく安定 ID を元に決定する。

理由:

- 名称変更で色が変わると視覚的識別が崩れるため。
- 利用者が一覧やカレンダー上で色を手がかりに認識するため。
- 同じデータを再取得しても安定した表示にするため。

生成方針:

1. client_id または project_id を文字列化する。
2. SHA-256 など安定したハッシュを計算する。
3. ハッシュ値を固定カラーパレットの index へ map する。
4. 背景色を default_icon_background_color に保存する。
5. 以降、名称変更では背景色を変更しない。

### 5.4 文字色生成

文字色は背景色とのコントラストを基準に決定する。

- 背景色が明るい場合は濃色を使用する。
- 背景色が暗い場合は白を使用する。
- WCAG のコントラスト比を考慮する。
- 固定候補は #FFFFFF と #111827 などとする。

### 5.5 名称変更時の扱い

| 対象 | icon_type | 方針 |
| --- | --- | --- |
| クライアント名変更 | DEFAULT | default_icon_text のみ再生成し、背景色は維持する |
| クライアント名変更 | UPLOADED | アップロード画像を維持する |
| 案件名変更 | DEFAULT | default_icon_text のみ再生成し、背景色は維持する |
| 案件名変更 | UPLOADED | アップロード画像を維持する |
...
6. default_icon_text を現在の名称から再生成する。
7. default_icon_background_color は維持する。欠損時のみ再生成する。
8. default_icon_text_color を背景色から再計算する。
9. version を更新する。
...
2. default_icon_text を新名称から再生成する。
3. default_icon_background_color は維持する。
4. default_icon_text_color は必要に応じて再計算する。
5. version を更新する。

UPLOADED アイコンの場合:

- アップロード済み画像を維持する。
- default_icon_text はフォールバック用として更新してもよい。
- icon_file_id は変更しない。
