...
## 24. Phase1 設定管理（2026-07-21追加）

- 設定画面はプロフィール、組織、事業情報、表示設定の4領域を扱う。
- プロフィールでは表示名、姓、名、電話番号、自己紹介を本人が更新できる。emailはPhase1では表示専用とする。
- 組織名と組織タイムゾーン、事業情報はOWNERだけが更新できる。ADMIN/MEMBERは基本情報を閲覧できる。
- 事業情報は屋号、郵便番号、都道府県、住所、適格請求書発行事業者番号、将来請求用既定税率を保持する。
- 既定税率は既存契約・精算計算へ影響させない。
- 表示設定は利用者タイムゾーン、週開始曜日、12/24時間表示、コンパクト表示を保持し、関連画面へ反映する。
- 通知設定、銀行口座、email変更はPhase2以降とする。
