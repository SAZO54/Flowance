...
- クライアント名から頭文字を生成する
- 日本語名では先頭の表示可能な1文字を使用する
- 英数字名では先頭文字またはイニシャルを使用する
- 背景色を自動設定する
- 背景色の上に頭文字を表示する
- 同じクライアントでは常に同じ初期色を再現できるようにする
- 文字色は背景色とのコントラストを確保する
- クライアント名変更時の色と頭文字の更新方針を統一する
...
日額契約、月額固定契約、成果報酬契約は Phase2 以降で検討する。
...
| 初期アイコン色の具体アルゴリズム | IDベース、名前ベース、固定パレットなど |
| クライアント名変更時の初期アイコン再生成 | 文字のみ変更、色も変更、既存維持 |
| 案件名変更時の初期アイコン再生成 | 文字のみ変更、色も変更、既存維持 |
