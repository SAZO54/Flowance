...
## 20. Settingsテスト（2026-07-21追加）

- GETが固定値ではなくDB上のprofile、organization、business、appearance、versionsを返すこと。
- profileのみ、appearanceのみ、全領域同時更新が成功すること。
- 省略領域は不変、空文字は解除、defaultTaxRateのnullは未設定として扱うこと。
- 入力不正、OWNER権限違反、いずれかのversion競合時に全更新がロールバックされること。
- Cookie認証PATCHのCSRF、組織境界、監査ログの機微情報非保存を確認すること。
- 設定画面の取得前空表示、タブ間の未保存値維持、取消、409再取得、保存後のサイドバー・日時・カレンダー・表示密度反映を確認すること。
