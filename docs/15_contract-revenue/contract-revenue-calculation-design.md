...
- 契約APIの失敗は契約パネル内で表示し、案件情報を隠さず再取得できる
- 登録・編集・削除後は案件詳細へ戻る
- MEMBERは閲覧のみ、OWNER/ADMINは登録・編集・削除できる
- 現在契約はisCurrent、履歴はvalid_from降順で表示する
- 契約未登録時は空状態と登録導線を表示する
- 登録・編集は案件配下の専用ページで行い、案件登録画面には含めない
- 案件詳細に現在の契約と契約履歴を表示する
契約は案件詳細を入口として扱うが、コード・API・DB処理は独立したContracts機能とする。
### 13.4 案件詳細内の契約管理
現在契約は組織タイムゾーンの当日を基準に、ACTIVEかつvalid_from以前、valid_untilがnullまたは当日以降で判定する。APIはisCurrentを返す。
### 13.3 現在契約判定
## 14. 状態遷移
...
未確定精算で参照されている契約は409 CONTRACT_IN_USEで論理削除を拒否する。確定済み精算はcalculation_snapshotとcontract_id参照を保持したまま契約の論理削除を許可する。
...
| 2026-07-20 | 1.1 | 案件詳細内の契約管理、現在契約判定、権限、未確定・確定精算参照時の削除条件を反映 |
