...
- 最大文字数を超えても入力自体は継続可能とし、リアルタイムバリデーションメッセージの表示と送信抑止で制御する。ブラウザの `maxlength` による入力打ち切りは使用しない。
- 最大文字数が定義された textarea は右下に `現在文字数／最大文字数` のカウンターを常時表示し、入力に合わせて即時更新する。上限超過時はカウンターもエラー色で表示する。
