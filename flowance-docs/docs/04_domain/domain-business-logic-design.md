...
| Infrastructure | Django ORM、PostgreSQL、Redis、Storage、Background Worker、外部技術連携 |
...
| ACCOUNTANT | Phase1対象外。Phase2以降で精算・請求・入金向けに検討 |
...
- クライアント、案件、契約は閲覧できる。
- 自身の予定と稼働実績は登録・編集できる。
- マスタ、契約、精算の変更は OWNER / ADMIN のみに許可する。
- 案件単位の細粒度な編集権限は Phase3 で扱う。
...
- 金額計算は Python の decimal.Decimal と Django DecimalField を使用し、浮動小数点を使用しない。
