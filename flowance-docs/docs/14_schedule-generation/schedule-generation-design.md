...
- 週次予定と個別予定はいずれも開始時刻が終了時刻より前でなければならない。
- breakMinutes は 0 以上、予定時間より短い値でなければならない。
- breakMinutes が予定時間と同じ、または予定時間を超える場合は実働 0 分以下の予定になるため無効とする。
- 週次予定と個別予定で breakMinutes の境界仕様を揃える。
